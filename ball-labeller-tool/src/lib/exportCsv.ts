...
export function exportCsv(meta: VideoMeta, labels: Map<number, LabelRecord>): void {
  ...
  const version = hasShotType ? 2 : hasImpact ? 1 : 0

  const header = `# version: ${version}\nframe,time_s,play_state,visibility,x,y,impact,shot_type,hand\n`
  const rows = sorted.map((r) => {
    const time_s = (r.frame / meta.fps).toFixed(4)
    return `${r.frame},${time_s},${r.play_state},${r.visibility},${r.x ?? ''},${r.y ?? ''},${r.impact ?? ''},${r.shot_type ?? ''},${r.hand ?? ''}`
  })
